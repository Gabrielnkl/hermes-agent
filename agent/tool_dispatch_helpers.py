...
# Delegated child output can carry attacker-controlled research back into the
# parent conversation: treating it as untrusted-data (wrapping + provenance
# taint on high-risk findings) closes the child -> parent laundering path.
_UNTRUSTED_TOOL_NAMES = frozenset({"web_extract", "web_search", "delegate_task"})
...
def _tool_output_risk_metadata(name: str, content: Any) -> Optional[Dict[str, Any]]:
    ...
    ``execute_code`` output is classified like untrusted output: only the risk
    metadata/taint path is extended — delimiter wrapping still applies solely
    to :func:`_is_untrusted_tool`, so model-visible content is unchanged.
    """
