...
def _build_child_agent(
    ...
    # Delegation inheritance: a tainted parent turn must not launder
    # provenance through a fresh child session. Inherit the parent's active
    # taint into the child's initial turn (one-shot consume at the child's
    # turn start). Findings come from the ambient parent taint record —
    # model-controlled delegation arguments cannot influence this.
    # Fail-closed: a staging failure on a known-tainted parent raises here,
    # failing this delegation tool call with an error rather than starting
    # a silent clean child. Clean parents return False and delegation
    # proceeds unchanged.
