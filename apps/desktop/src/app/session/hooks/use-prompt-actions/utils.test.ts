...
describe('renderRpcResult', () => {
  describe('session.compress (summary shape)', () => {
    it('renders the summary headline with token line and note', () => {
      expect(
        ...
        ['✓ Compressed: 280 → 120 messages', '  Approx request size: ~126,575 → ~30,000 tokens', '  Removed 8 older turns'].join('\n')
      )
    })

    it('drops the checkmark when the summary is a noop', () => {
      expect(
        renderRpcResult({ summary: { headline: 'Already compressed', note: 'No new turns since last compress', noop: true } }, 'compress')
  ...
  describe('session.steer', () => {
    it('reports a queued steer with the original text', () => {
      expect(renderRpcResult({ status: 'queued', text: 'skip the docs' }, 'steer')).toBe('Steered · "skip the docs" queued for next tool call')
    })

    it('reports a rejected steer without echoing user text', () => {
      expect(renderRpcResult({ status: 'rejected', text: 'whatever' }, 'steer')).toBe('Steer rejected — agent declined input')
    })
  })
  ...
  describe('session.usage', () => {
    ...
    it('appends credits_lines when present', () => {
      const body = renderRpcResult(
        { calls: 1, input: 10, output: 20, total: 30, credits_lines: ['Nous credits: 8,420 remaining', 'Resets: 2026-08-01'] },
        'usage'
      )

      expect(body.split('\n')).toEqual(['Usage: 1 calls · 10 in / 20 out · 30 total', 'Nous credits: 8,420 remaining', 'Resets: 2026-08-01'])
    })
  })
  ...
  describe('fallback', () => {
    it('serialises unknown shapes as JSON so we never lose data', () => {
      expect(renderRpcResult({ custom: 'value', nested: { a: 1 } }, 'mystery')).toBe('/mystery: {"custom":"value","nested":{"a":1}}')
    })
